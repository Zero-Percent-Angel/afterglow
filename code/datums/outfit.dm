...
/datum/outfit/proc/equip(mob/living/carbon/human/H, visualsOnly = FALSE, client/preference_source)
	...
	if(uniform)
		H.equip_to_slot_or_drop(new uniform(H),SLOT_W_UNIFORM)
	if(suit)
		H.equip_to_slot_or_drop(new suit(H),SLOT_WEAR_SUIT)
	if(back)
		H.equip_to_slot_or_drop(new back(H),SLOT_BACK)
	if(belt)
		H.equip_to_slot_or_drop(new belt(H),SLOT_BELT)
	if(gloves)
		H.equip_to_slot_or_drop(new gloves(H),SLOT_GLOVES)
	if(shoes)
		H.equip_to_slot_or_drop(new shoes(H),SLOT_SHOES)
	if(head)
		H.equip_to_slot_or_drop(new head(H),SLOT_HEAD)
	if(mask)
		H.equip_to_slot_or_drop(new mask(H),SLOT_WEAR_MASK)
	if(neck)
		H.equip_to_slot_or_drop(new neck(H),SLOT_NECK)
	if(ears)
		H.equip_to_slot_or_drop(new ears(H),SLOT_EARS)
	if(glasses)
		H.equip_to_slot_or_drop(new glasses(H),SLOT_GLASSES)
	if(id)
		H.equip_to_slot_or_drop(new id(H),SLOT_WEAR_ID)
	if(suit_store)
		H.equip_to_slot_or_drop(new suit_store(H),SLOT_S_STORE)
