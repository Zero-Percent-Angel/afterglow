...
/mob/living/carbon/human/updatehealth()
	...
	if(!HAS_TRAIT(src, TRAIT_IGNOREDAMAGESLOWDOWN))	//if we want to ignore slowdown from damage, but not from equipment
		var/health_deficiency = ((maxHealth + stambufferinfluence) - health + (getStaminaLoss()*0.5))//CIT CHANGE - reduces the impact of staminaloss and makes stamina buffer influence it
