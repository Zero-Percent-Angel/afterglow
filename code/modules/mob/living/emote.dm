	...
	if(. && ishuman(user))
		...
		if(H.dna.features["wings"] != "None")
			if(H.dna.species.mutant_bodyparts["wingsopen"])
				...
				H.OpenWings()
			addtimer(CALLBACK(H, TYPE_PROC_REF(open ? /mob/living/carbon/human, OpenWings : /mob/living/carbon/human, CloseWings)), wing_time)
...
/datum/emote/living/audible/laugh/get_sound(mob/living/user)
	...
	if(ishuman(user))
		...
		if(istype(human_user.get_item_by_slot(ITEM_SLOT_OCLOTHING), /obj/item/clothing/suit/armor/tiered/power_armor))
