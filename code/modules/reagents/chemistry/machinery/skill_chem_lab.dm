...
/obj/machinery/chem_lab/Initialize(mapload)
	. = ..()
	dispensable_reagents = sortList(basic_chemicals, GLOBAL_PROC_REF(cmp_reagents_asc))
	if(advanced_chemicals)
		advanced_chemicals = sortList(advanced_chemicals, GLOBAL_PROC_REF(cmp_reagents_asc))
	if(expert_chemicals)
		expert_chemicals = sortList(expert_chemicals, GLOBAL_PROC_REF(cmp_reagents_asc))
	if(upgraded_chemicals)
		upgraded_chemicals = sortList(upgraded_chemicals, GLOBAL_PROC_REF(cmp_reagents_asc))
	if(upgraded_chemicals2)
		upgraded_chemicals2 = sortList(upgraded_chemicals2, GLOBAL_PROC_REF(cmp_reagents_asc))
