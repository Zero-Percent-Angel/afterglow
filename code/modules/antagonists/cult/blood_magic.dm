	...
	if(proximity_flag && iscultist(user))
		...
		if(istype(target, /obj/item/stack/sheet/metal))
			...
			if(candidate.use(quantity))
				...
				SEND_SOUND(user, sound('sound/effects/magic.ogg',0,1,25))
		if(istype(target, /obj/item/clothing/suit/armor/tiered/hooded/wintercoat) && target.type !=  /obj/item/clothing/suit/armor/tiered/hooded/wintercoat/narsie)
			if (do_after(user,30,target=target))
				new /obj/item/clothing/suit/armor/tiered/hooded/wintercoat/narsie(T)
				qdel(target)
