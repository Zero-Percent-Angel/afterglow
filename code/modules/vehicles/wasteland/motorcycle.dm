...
/obj/vehicle/ridden/fuel/motorcycle/Initialize(mapload)
	. = ..()
	update_icon()
	if (!istype(motorcycle))
		motorcycle = mutable_appearance(icon, icon_state + "_cover", ABOVE_MOB_LAYER, MOB_PLANE)
	var/datum/component/riding/D = LoadComponent(/datum/component/riding)
	D.vehicle_move_delay = 1
	D.set_riding_offsets(RIDING_OFFSET_ALL, list(TEXT_NORTH = list(1, 0), TEXT_SOUTH = list(1, 2), TEXT_EAST = list(-1, 5), TEXT_WEST = list( 2, 5)))
